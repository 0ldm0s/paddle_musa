...
COMMON_DECLARE_bool(enable_mublas_tensor_op_math);
...
// TODO(someone): keep below code disabled until musa sdk supports mem async api
  // /**
  //  * Try to allocate `size` gpu memory. Only musaErrorMemoryAllocation
  //  * or musaSuccess would be returned, and the musaGetLastError() flag
  //  * would be clear.
  //  */
  ...
  //   std::call_once(set_musamempoolattr_once_flag_, [&]() {
  //     PADDLE_ENFORCE_GPU_SUCCESS(
  //         muDeviceGetDefaultMemPool(&memPool_, dev_id_));
  //     uint64_t thresholdVal = FLAGS_cuda_memory_async_pool_release_threshold;
  //     VLOG(10) << "[musaMallocAsync] set musaMemPoolAttrReleaseThreshold to "
  //              << thresholdVal;
  //     PADDLE_ENFORCE_GPU_SUCCESS(
  //         muMemPoolSetAttribute(memPool_,
  //                                 musaMemPoolAttrReleaseThreshold,
  //                                 reinterpret_cast<void *>(&thresholdVal)));
  //   });

  //   gpuError_t result;
  //   result = musaMallocAsync(ptr, size, stream);
  //   VLOG(10) << "[musaMallocAsync] ptr = " << (*ptr)
  ...
  void Free(void *ptr, size_t size) {
    ...
    if (err != gpuErrorCudartUnloading) {
      ...
      platform::RecordMemEvent(
          ptr, phi::CustomPlace("musa", dev_id_), size, phi::TracerMemEventType::ReservedFree);
    } else {
...
//     // Purposefully allow musaErrorCudartUnloading, because
//     // that is returned if you ever call cudaFree after the
//     // driver has already shutdown. This happens only if the
//     // process is terminating, in which case we don't care if
//     // cudaFree succeeds.
//     CUDADeviceGuard guard(dev_id_);
//     auto err = musaFreeAsync(ptr, stream);
//     VLOG(10) << "[musaFreeAsync] ptr = " << ptr
//              << " size =" << static_cast<double>(size) / (1 << 20)
...
//   }
  ...
  MUresult MemCreate(MUmemGenericAllocationHandle *handle,
                     size_t size,
                     const MUmemAllocationProp *prop,
                     unsigned long long flags) {  // NOLINT
    auto result = phi::dynload::muMemCreate(handle, size, prop, flags);
  ...
  MUresult MemRelease(MUmemGenericAllocationHandle handle, size_t size) {
    auto result = phi::dynload::muMemRelease(handle);
 ...
 private:
  ...
  musaMemPool_t memPool_ = nullptr;
  static std::once_flag set_musamempoolattr_once_flag_;
...
std::once_flag RecordedGpuMallocHelper::set_musamempoolattr_once_flag_;
...
MUresult RecordedGpuMemCreate(MUmemGenericAllocationHandle *handle,
                              size_t size,
                              const MUmemAllocationProp *prop,
                              unsigned long long flags,  // NOLINT
                              int dev_id) {
...
MUresult RecordedGpuMemRelease(MUmemGenericAllocationHandle handle,
                               size_t size,
                               int dev_id) {
...
void EmptyCache() {
  ...
  for (auto device : devices) {
    memory::Release(phi::CustomPlace("musa", device));
  }
}

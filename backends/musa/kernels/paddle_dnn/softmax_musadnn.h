...
__global__ void NormalSoftmaxForward(T* output,
  ...
  for (IndexType high_id = blockIdx.y; high_id < high_dim;
       high_id += gridDim.y) {
    for (IndexType low_base =
             static_cast<IndexType>(blockIdx.x) * blockDim.x;
         low_base < low_dim;
         low_base += static_cast<IndexType>(blockDim.x) * gridDim.x) {
      const IndexType low_id = low_base + threadIdx.x;
      const bool active = low_id < low_dim;
...
bool UseCudnnSoftmax(const GPUContext& dev_ctx,
                     int64_t softmax_dim,
                     bool last_dim) {
  bool cudnn_available = (GetDnnHandle(dev_ctx.stream(), dev_ctx.GetPlace()) == nullptr);
  if (!cudnn_available) {

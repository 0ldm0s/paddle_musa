...
void CrossEntropyWithSoftmaxBwdWithDowncastGPUKernel(
  ...
  // MUSA custom backend uses a custom place type, so skip the CUDA-only
  // place check from the official GPU implementation.
...
PD_CUSTOM_KERNEL_REGISTER(cross_entropy_with_softmax_bwd_w_downcast,
                          musa,
                          ALL_LAYOUT,

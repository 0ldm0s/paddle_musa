...
void ExpandStrideKernel(const Context& dev_ctx,
  ...
  if (invalid_stride) {
    ...
    out->set_meta(meta);
    phi::ExpandKernel<T, Context>(dev_ctx, x, shape, out);
    return;
  }
  ...
  auto meta = out->meta();
  meta.dims = DDim(expandedSizes.data(), static_cast<int>(expandedSizes.size()));
  meta.strides =
...
PD_CUSTOM_KERNEL_REGISTER(expand_stride,
                          musa,
                          STRIDED,

...
__host__ __device__ inline float2 bfloat1622float2(const __mt_bfloat162 a) {
// #if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800))
  return __bfloat1622float2(a);
// // #else
//    float hi_float;
//    float lo_float;
//    lo_float = __internal_bfloat162float(((__mt_bfloat162_raw)a).x);
//    hi_float = __internal_bfloat162float(((__mt_bfloat162_raw)a).y);
//    return make_float2(lo_float, hi_float);
// // #endif
}

__host__ __device__ inline __mt_bfloat162 float22bfloat162_rn(const float2 a) {
  __mt_bfloat162 val;
// #if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800))
...
inline __device__ void UpdateSum<phi::bfloat16, 2>(const phi::bfloat16* srcX,
                                                   float* sum,
                                                   float* sumSq) {
  __mt_bfloat162 h2 = *reinterpret_cast<__mt_bfloat162 const*>(srcX);
...
inline __device__ void UpdateSum<phi::bfloat16, 2>(const phi::bfloat16* srcX,
                                                   ...
                                                   float* sumSq) {
  __mt_bfloat162 h2 = *reinterpret_cast<__mt_bfloat162 const*>(srcX);
  __mt_bfloat162 h2_r = *reinterpret_cast<__mt_bfloat162 const*>(srcR);
  float2 f2 = phi::bfloat1622float2(h2);
...
inline __device__ void GroupNormCompute<phi::bfloat16, 2>(
    ...
    float invStdDev) {
  float2 gammaF2, betaF2;
  gammaF2 = phi::bfloat1622float2(*reinterpret_cast<__mt_bfloat162 const*>(
      reinterpret_cast<__mt_bfloat16 const*>(params.gamma) + ci));
  betaF2 = phi::bfloat1622float2(*reinterpret_cast<__mt_bfloat162 const*>(
      reinterpret_cast<__mt_bfloat16 const*>(params.beta) + ci));
  ...
  for (int64_t dhwi = dhwBegin; dhwi < dhwEnd; ++dhwi) {
    ...
    // Fetch two channels per thread.
    __mt_bfloat162 h2 =
        *reinterpret_cast<__mt_bfloat162 const*>(&params.srcX[offset]);
    ...
    if (params.srcR != nullptr) {
      ...
      int64_t g_offset =
          params.y_same_with_x ? offset : gi * params.cPerGroup + gj;
      __mt_bfloat162 r2 =
          *reinterpret_cast<__mt_bfloat162 const*>(&params.srcR[g_offset]);
      float2 r_f2 = phi::bfloat1622float2(r2);
      f2.x += r_f2.x;
      f2.y += r_f2.y;
      *reinterpret_cast<__mt_bfloat162*>(&params.eleOut[offset]) =
    ...
    // Store the scaled values.
    *reinterpret_cast<__mt_bfloat162*>(&params.dst[offset]) =
        phi::float22bfloat162_rn(f2);
  }
}
#endif
...
void GroupNormKernel(const Context& dev_ctx,
  ...
  if (data_layout_str != "NCHW") {
     printf("group norm only support NCHW layout, current layout is %s", data_layout_str.c_str());
     throw -11; 
  }
  if (y && y->numel() == 0) {
    dev_ctx.template Alloc<T>(y);

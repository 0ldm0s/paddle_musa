...
void GPUCollectFpnProposalsOpKernel(
  ...
  std::vector<int> out_id_cpu(real_post_num);
  phi::memory_utils::Copy(phi::CPUPlace(),
                          out_id_cpu.data(),
                          place,
                          out_id_data,
                          sizeof(int) * real_post_num,
                          dev_ctx.stream());
  dev_ctx.Wait();

  std::vector<int> length_lod_host(lod_size, 0);
  for (int i = 0; i < real_post_num; ++i) {
    int v = out_id_cpu[i];
    PADDLE_ENFORCE_GE(
        v,
        0,
        common::errors::InvalidArgument(
            "The batch id in collect_fpn_proposals is negative: %d.", v));
    PADDLE_ENFORCE_LT(
        v,
        lod_size,
        common::errors::InvalidArgument(
            "The batch id in collect_fpn_proposals is out of range: %d vs lod_size %d.",
            v,
            lod_size));
    length_lod_host[v] += 1;
  }

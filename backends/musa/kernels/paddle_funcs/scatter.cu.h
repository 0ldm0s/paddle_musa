...
void GPUScatterAssign(const phi::GPUContext& dev_ctx,
  ...
  if (!overwrite) {
    ...
    // return;
    if constexpr (std::is_same<T, phi::dtype::bfloat16>::value) {
      DenseTensor src_fp32;
      src_fp32.Resize(src.dims());
      dev_ctx.template Alloc<float>(&src_fp32);

      DenseTensor out_fp32;
      out_fp32.Resize(output->dims());
      dev_ctx.template Alloc<float>(&out_fp32);

      phi::CastKernel<T, phi::GPUContext>(
          dev_ctx, src, phi::DataType::FLOAT32, &src_fp32);

      phi::CastKernel<T, phi::GPUContext>(
          dev_ctx, *output, phi::DataType::FLOAT32, &out_fp32);

      const float* p_src_fp32 = src_fp32.data<float>();
      float* p_out_fp32 = out_fp32.data<float>();

      ScatterInitCUDAKernel<float, IndexT><<<grid, block, 0, dev_ctx.stream()>>>(
          p_index, p_out_fp32, output_dims[0], index_size, slice_size);

      ScatterCUDAKernel<float, IndexT, false, 1>
          <<<grid, block, 0, dev_ctx.stream()>>>(
              p_src_fp32,
              p_index,
              p_out_fp32,
      ...
      phi::CastKernel<float, phi::GPUContext>(
          dev_ctx, out_fp32, phi::DataType::BFLOAT16, output);

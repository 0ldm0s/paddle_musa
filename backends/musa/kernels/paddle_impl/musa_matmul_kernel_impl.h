...
#if defined(PADDLE_WITH_MUSA) && CUDA_VERSION >= 11060
...
bool MatMulInt8Function(const Context& dev_ctx,
                        const DenseTensor& x,
                        const DenseTensor& y,
                        const std::vector<std::int64_t>& x_dims,
                        const std::vector<std::int64_t>& y_dims,
...
MatmulJudgeDtypeKernel(const Context& dev_ctx,
                       const DenseTensor& x,
                       const DenseTensor& y,
                       const std::vector<std::int64_t>& x_dims,
                       const std::vector<std::int64_t>& y_dims,
...
void MatmulWithFlattenKernelImpl(const Context& dev_ctx,
                                 ...
                                 int y_num_col_dims,
                                 DenseTensor* out) {
  // TODO(jihong.zhong): need to fix below

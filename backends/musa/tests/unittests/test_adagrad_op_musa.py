...
class TestAdagradMultiPrecision2_0(unittest.TestCase):
    ...
    def static_adagrad_mp(self, mp, use_amp):
        ...
        with paddle.static.program_guard(train_program, startup_program):
            ...
            bias_init = np.zeros([10], dtype="float32")

            hidden_layer = paddle.nn.Linear(
                2,
                10,
                weight_attr=paddle.ParamAttr(
                    initializer=paddle.nn.initializer.Assign(weight_init)
                ),
                bias_attr=paddle.ParamAttr(
                    initializer=paddle.nn.initializer.Assign(bias_init)
                ),
            )
            if use_amp:

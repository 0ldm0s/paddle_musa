...
class TestSlogDetGradOpMusa(unittest.TestCase):
    def test_registered(self):
        paddle.set_device("musa")
        kernels = core._get_registered_phi_kernels()
        for op_name in ["slogdet_grad", "slogdet_v2_grad"]:
            self.assertIn(op_name, kernels)
            self.assertTrue(any("musa" in kernel for kernel in kernels[op_name]))

    def test_functional_slogdet_v2_grad(self):
        paddle.set_device("musa")
        x_np = np.array([[1.0, 2.0], [3.0, 5.0]], dtype="float32")
        x = paddle.to_tensor(x_np, stop_gradient=False)
        _, logabsdet = paddle.linalg.slogdet(x)
        logabsdet.backward()

        expected = np.linalg.inv(x_np).T

...
Paddle MUSA 当前适配摩尔线程 S5000 显卡，围绕飞桨框架与 S5000 硬件能力之间的全链路打通，覆盖设备发现与管理、运行时调度、算子 kernel 注册与调用、MUDNN/MCCL 等基础库集成、分布式通信、性能分析以及模型验证等关键环节。通过自定义的[规则化机制](backends/musa/hack/README.md)管理和 CUDA 的差异，降低 Paddle 主线代码升级适配成本。
...
| 算子适配规模 | 972 类算子 |
| 算子适配覆盖率 | 91.38%（以 Paddle GPU/CUDA 1067 类算子为 100% 基准统计） |
| 模型适配规模 | 44 个飞桨生态模型 |
| 覆盖领域 | 6 类模型与应用场景 |
